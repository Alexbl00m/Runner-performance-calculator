...
def calculate_running_zones(threshold_pace_sec):
    """Calculate Lindblom coaching running zones based on threshold pace in seconds per km"""
    zones = {
...
# Tab 2: Training Zones
with tab2:
    st.header("Training Zones Calculator")
    st.markdown("""
    <div class="result-box">
    View your optimal training zones based on your running performances. These zones are calculated using the Lindblom Coaching method, which uses your threshold pace as a reference.
    </div>
    """, unsafe_allow_html=True)
    ...
    if 'distances' in locals() and 'times' in locals() and len(distances) > 0 and len(times) > 0:
        ...
        # Calculate Lindblom Coaching running zones
...
# Tab 3: Race Pace Calculator - Fixed version
with tab3:
    ...
    with col1:
        ...
        if pace_strategy == "Even Pace":
            ...
            split_factor = st.slider("First Half Adjustment:", min_value=0.01, max_value=0.10, value=0.05, step=0.01, format="+%g", key="neg_split")
            st.info(f"First half {split_factor:.0%} slower, second half {split_factor:.0%} faster")
        else:  # Positive Split
            split_factor = st.slider("First Half Adjustment:", min_value=0.01, max_value=0.10, value=0.05, step=0.01, format="-%g", key="pos_split")
    ...
    with col2:
        ...
        if input_method == "Target Pace":
            pace_unit = st.radio("Pace Unit", ["min/km", "min/mile"], horizontal=True, key="pace_unit")
            
            if pace_unit == "min/km":
                pace_input = st.text_input("Target Pace (min:sec per km)", "5:00", key="pace_km_input")
                try:
                ...
                pace_input = st.text_input("Target Pace (min:sec per mile)", "8:00", key="pace_mile_input")
                try:
                    pace_parts = pace_input.split(":")
                    if len(pace_parts) == 2:
                        pace_min, pace_sec = int(pace_parts[0]), int(pace_parts[1])
            ...
            with col_h:
                hours = st.number_input("Hours", min_value=0, max_value=24, value=0, key="hours_input")
            with col_m:
                minutes = st.number_input("Minutes", min_value=0, max_value=59, value=20, key="minutes_input")
            with col_s:
                seconds = st.number_input("Seconds", min_value=0, max_value=59, value=0, key="seconds_input")
    ...
    if pace_seconds_per_km > 0 and race_distance_km > 0:
        ...
        splits_data = []
        splits_paces_numeric = []  # For the chart
        split_labels = []  # For the chart
        ...
        for i in range(1, num_splits + 1):
            ...
            split_labels.append(f"{i} ({split_label})")
            splits_paces_numeric.append(adjusted_pace / 60)  # Convert to minutes for chart
        ...
        if partial_split > 0.01:  # Only if it's significant
            ...
            split_labels.append(f"Final ({partial_split*1000:.0f}m)")
            splits_paces_numeric.append(adjusted_pace / 60)  # Convert to minutes for chart
        ...
        # Create split visualization data
        target_pace_min = pace_seconds_per_km / 60  # Convert to minutes for the chart
        
        # Create figure
        fig = go.Figure()
        
        # Add bar chart for split paces
        fig.add_trace(go.Bar(
            x=split_labels,
            y=splits_paces_numeric,
            name="Split Paces",
            text=[f"{min}:{int((min % 1) * 60):02d}" for min in splits_paces_numeric],
            textposition="auto",
            marker_color="#E6754E"
        ))
        ...
        fig.add_annotation(
            ...
            y=target_pace_min,
            text=f"Target Pace: {int(target_pace_min)}:{int((target_pace_min % 1) * 60):02d}",
            showarrow=False,
            yshift=10,
            font=dict(color="red")
        )
        
        # Set y-axis format to show minutes correctly
        fig.update_layout(
            ...
            yaxis=dict(
                tickmode='array',
                tickvals=[i/2 for i in range(int(target_pace_min*2-2), int(target_pace_min*2+6))],
                ticktext=[f"{int(i/2)}:{int((i/2 % 1) * 60):02d}" for i in range(int(target_pace_min*2-2), int(target_pace_min*2+6))]
            ),
            template="plotly_white",
            height=400
        )
